                ...
                <div id="content">
                    <%
                                if (request.getAttribute("INFO") == null) {
                                    request.getRequestDispatcher("EventShowServlet?txtFrom=01/01/1990&txtTo=12/30/2100&action=Search").forward(request, response);
                                }
                    ...
                    <div class="topImg clearfix">
                        ...
                        <p>Events <strong>Show</strong></p>
                    </div>
                    <div class="wrapper">
                        <div id="product-table" class="welcome column c-67 clearfix">
                            <%
                                        
                                        for (int i = 0; i < eventList.length; i++) {
                                            cal.setTime(eventList[i].getEventDate());
                            ...
                            <ul>
                                <div id="TitleEvent"><h3>Event <%=i + 1%> : <%=eventList[i].getEventName()%> </h3>
                                    <h3><%if (eventList[i].getEventStt() == 1) {%>Status: Happened<% } else if (eventList[i].getEventStt() == 2) {%>Status: Cancel<%} else {%>Status : Not yet Happened<% }%> || Begin :<%=date%></h3>
                                </div>

                                <div class="cContent clearfix">
                                    <img id="EventShowImages" src="images/event/<%=eventList[i].getEventImg()%>" alt="">
                                    <div>
                                        <p><%=eventList[i].getEventDes()%>.<a href="EventDetailServlet?EventID=<%=eventList[i].getEventID()%>"> Read more</a></p>
                                    </div>
                                    <div>
                                        <form action="EventShowServlet" method="post">
                                            <input type="hidden" name="eventID" value="<%=eventList[i].getEventID()%>"/>
                                            <%if (cal.getTime().after(calCrr.getTime()) && isLoggedin) {%>
                                            <input id="EnrollBT" class="submit" type="submit" value="Enroll" name="action"/>
                                            <%}%>
                                        </form>
                                    </div>
                                </div>
                        ...
                        <div class="searchCourse searchCourseHome column c-33 clearfix">
                            ...
                            <form action="HomeServlet" method="post">
                                ...
                                <input class="submit" name="action" type="submit" value="Search" />
                            </form>
                        </div>
