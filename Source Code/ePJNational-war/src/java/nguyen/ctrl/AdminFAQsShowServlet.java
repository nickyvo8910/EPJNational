 ...
 * @author KieuTrinh
 */
public class AdminFAQsShowServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (action.equals("Search")) {
                ...
                TblFAQ[] faq = local.getFAQList();
                request.setAttribute("INFOFAQ", faq);
                request.getRequestDispatcher("AdminFAQShow.jsp").forward(request, response);
            } else if (action.equals("Edit")) {
                String id = request.getParameter("FAQid");
                request.setAttribute("ID", id);
                request.getRequestDispatcher("AdminFAQUpdateServlet?action=Edit").forward(request, response);
            } else if (action.equals("Delete")) {
                int id = Integer.parseInt(request.getParameter("FAQid"));
                if (local.updateFAQStt(id) == 1) {
                    request.getRequestDispatcher("AdminFAQShow.jsp").forward(request, response);
                } else {
