...
public class AdminFAQUpdateServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String url ="AdminFAQShow.jsp";
            
                if (action.equals("Submit")) {
                    String question = request.getParameter("txtQuestion");
                    String answer = request.getParameter("txtAnswer");
                    int faqID = Integer.parseInt(request.getParameter("txtId"));
                    int result = local.updateFAQ(faqID, question, answer);
                    if (result == 1) {
                        request.setAttribute("alert", "Update Successful");
                        url="AdminFAQShow.jsp";
                    }else{
                        request.setAttribute("alert", "Update Unsuccessful");
                        url = "AdminFAQUpdate.jsp";
                    }
                } else if (action.equals("Edit")) {
                    String id = (String) request.getAttribute("ID");
                    int EventID = Integer.parseInt(id);
                    TblFAQ record = local.getFAQ(EventID);
